...
struct BlockQuotationTests {
    ...
    func blockQuoteElementConformsToHTMLElement() {
        let blockquote = BlockQuote()
        let element: any HTMLElement = blockquote
    ...
    struct IntegrationTests {
        ...
        func blockQuoteWithComplexCiteURLs() {
            let complexCite = Cite(value: "https://example.com/article?id=123&section=conclusion#paragraph-5")
            let blockquote = BlockQuote(cite: complexCite)
            #expect(blockquote.cite?.rawValue == "https://example.com/article?id=123&section=conclusion#paragraph-5")
        }
    }
